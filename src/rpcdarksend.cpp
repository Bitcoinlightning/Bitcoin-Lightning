...
void SendMoney(const CTxDestination &address, CAmount nValue, CWalletTx& wtxNew, AvailableCoinsType coin_type=ALL_COINS)
{
    ...
    // Parse Bitcoin_Lightning address
...
Value darksend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "darksend <Bitcoin_Lightningaddress> <amount>\n"
            "Bitcoin_Lightningaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "darksend <Bitcoin_Lightningaddress> <amount>\n"
            "Bitcoin_Lightningaddress, denominate, or auto (AutoDenominate)"
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw runtime_error(
                ...
                "  start        - Start masternode configured in Bitcoin_Lightning.conf\n"
                "  start-alias  - Start single masternode by assigned alias configured in masternode.conf\n"
                "  start-many   - Start all masternodes configured in masternode.conf\n"
                "  status       - Print masternode status information\n"
                "  stop         - Stop masternode configured in Bitcoin_Lightning.conf\n"
                ...
                "  vote-many    - Vote on a Bitcoin_Lightning initiative\n"
                "  vote         - Vote on a Bitcoin_Lightning initiative\n"
                );
    ...
    if (strCommand == "genkey")
    {
        ...
        return CBitcoin_LightningcoinSecret(secret).ToString();
    }
...
Value masternodelist(const Array& params, bool fHelp)
{
                ...
                && strMode != "pubkey" && strMode != "rank" && strMode != "status" && strMode != "addr" && strMode != "votes" && strMode != "lastpaid"))
    {
        throw runtime_error(
                ...
                "  votes          - Print all masternode votes for a Bitcoin_Lightning initiative (can be additionally filtered, partial match)\n"
