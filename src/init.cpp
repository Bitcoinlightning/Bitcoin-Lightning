...
void Shutdown()
{
    ...
    RenameThread("Bitcoin_Lightning-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Bitcoin_Lightning.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Bitcoin_Lightningd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeBitcoin_Lightningamount=<n> " + _("Keep N Bitcoin_Lightning anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Bitcoin_Lightning is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Bitcoin_Lightning is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Bitcoin_Lightning version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Bitcoin_Lightning server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Bitcoin_Lightning") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Bitcoin_Lightning to complete") << "\n";
    ...
    nAnonymizeBitcoin_LightningAmount = GetArg("-anonymizeBitcoin_Lightningamount", 0);
    if(nAnonymizeBitcoin_LightningAmount > 999999) nAnonymizeBitcoin_LightningAmount = 999999;
    if(nAnonymizeBitcoin_LightningAmount < 2) nAnonymizeBitcoin_LightningAmount = 2;
    ...
    LogPrintf("Anonymize Bitcoin_Lightning Amount %d\n", nAnonymizeBitcoin_LightningAmount);
