...
int nAnonymizeBitcoin_LightningAmount = 1000;
...
#else
    const char* pszModule = "Bitcoin_Lightning";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Bitcoin_Lightning
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Bitcoin_Lightning
    // Mac: ~/Library/Application Support/Bitcoin_Lightning
    // Unix: ~/.Bitcoin_Lightning
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bitcoin_Lightning";
#else
    fs::path pathRet;
    ...
    return pathRet / "Bitcoin_Lightning";
#else
    // Unix
    return pathRet / ".Bitcoin_Lightning";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Bitcoin_Lightning.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Bitcoin_Lightningd.pid"));
