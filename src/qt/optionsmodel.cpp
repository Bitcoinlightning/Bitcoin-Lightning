...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeBitcoin_LightningAmount"))
        settings.setValue("nAnonymizeBitcoin_LightningAmount", 1000);
    nAnonymizeBitcoin_LightningAmount = settings.value("nAnonymizeBitcoin_LightningAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeBitcoin_LightningAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeBitcoin_LightningAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeBitcoin_LightningAmount:
            return QVariant(nAnonymizeBitcoin_LightningAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeBitcoin_LightningAmount:
            nAnonymizeBitcoin_LightningAmount = value.toInt();
            settings.setValue("nAnonymizeBitcoin_LightningAmount", nAnonymizeBitcoin_LightningAmount);
            emit AnonymizeBitcoin_LightningAmountChanged(nAnonymizeBitcoin_LightningAmount);
            break;
        default:
            break;
        }
