...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("Bitcoin_Lightning"))
        return false;
...
{
    // Convert Bitcoin_Lightning:// to Bitcoin_Lightning:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Bitcoin_Lightning://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Bitcoin_Lightning:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Bitcoin_Lightning.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Bitcoin_Lightning.lnk
...
{
    return GetAutostartDir() / "Bitcoin_Lightning.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Bitcoin_Lightning.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Bitcoin_Lightning\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Bitcoin_Lightning app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Bitcoin_Lightning app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Bitcoin_Lightning-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Bitcoin_Lightning-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Bitcoin_Lightning-Qt"));
