...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeBitcoin_LightningAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeBitcoin_LightningAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeBitcoin_LightningAmount = strAnonymizeBitcoin_LightningAmount.remove(strAnonymizeBitcoin_LightningAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBitcoin_LightningAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeBitcoin_LightningAmount*COIN) nMaxToAnonymize = nAnonymizeBitcoin_LightningAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeBitcoin_LightningAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeBitcoin_LightningAmount));
        strAnonymizeBitcoin_LightningAmount = strAnonymizeBitcoin_LightningAmount.remove(strAnonymizeBitcoin_LightningAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBitcoin_LightningAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeBitcoin_LightningAmount == 0){
